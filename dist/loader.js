...
const processLoaderForSources = memoize_1.default(async (sourceFullPath, sourceContent, addDependency, loaderCwd, { configFile }) => {
    const [config, configHash] = await config_1.default(loaderCwd, configFile);
...
const processLoaderForDocuments = memoize_1.default(async (gqlFullPath, gqlContent, addDependency, loaderCwd, { configFile }) => {
    const [config, configHash] = await config_1.default(loaderCwd, configFile);
    const cwd = config.cwd;
    const { silent } = config;
